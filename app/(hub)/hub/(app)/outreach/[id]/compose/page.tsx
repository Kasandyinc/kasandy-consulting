...
const STEPS = [
  ...
  { key: 'PHONE', templateId: 'O-02', label: 'Phone script', channel: 'manual' as const },
...
export default async function ComposePage({
  ...
  const steps: StepView[] = STEPS.map((s) => {
    ...
    return {
      ...
      subjects: draft?.subjects ?? (template?.subject ? [template.subject] : []),
      bodyMd: draft?.body_md ?? template?.body_md ?? '',
